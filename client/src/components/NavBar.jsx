...
const NavBar = ({ topPage, selectedPage, setSelectedPage }) => {
  ...
  if (isHomePage) {
    return (
      <nav
        className={`${backgroundColor} max-w-full fixed top-0 z-2999 w-full p-4 mb-3`}
      >
          ...
          <div className="">
            <article className=" md:border-l-2 divide-orange-600 p-4 grid grid-cols-3 place-items-center">
              <div className="hidden md:flex flex-col items-center  cursor-pointer">
                <span>{lang}</span>
                <ArrowDropDownIcon onClick={() => setOpenLang(!openLang)} />
                {openLang && (
                  <>
                    {lang !== "ESP" && <span onClick={()=>handleLanguage("ESP")}>ESP</span>}
                    {lang !== "NL" && <span onClick={()=>handleLanguage("NL")}>NL</span>}
                    {lang !== "ENG" && <span onClick={()=>handleLanguage("ENG")}>ENG</span>}
                  </>
                )}
              </div>
